...
export default function App() {
  ...
  async function saatPilihKota(kota) {
    ...
    try {
      ...
      if (!cuaca) {
        setPesanError("Data kota tidak ditemukan dalam database lokal.");
        setCuacaSekarang(null);
        setPrakiraan(null);
      } else {
        ...
        setRiwayat((lama) => {
          ...
          if (sudahAda)
            return [item, ...lama.filter((p) => p.key !== key)].slice(0, 10);
          return [item, ...lama].slice(0, 10);
        });
      }
    } catch {
      setPesanError("Gagal memuat data cuaca dari file lokal.");
    } finally {
      setMemuat(false);
    }
  }
  ...
  return (
    <div className="app-container">
      ...
      <div className="top-controls">
        ...
        <div className="unit-toggle">
          <label>
            <input
              type="radio"
              checked={satuan === "metric"}
              onChange={() => setSatuan("metric")}
            />{" "}
            °C
          </label>
          <label style={{ marginLeft: 8 }}>
            <input
              type="radio"
              checked={satuan === "imperial"}
              onChange={() => setSatuan("imperial")}
            />{" "}
            °F
          </label>
        </div>
      ...
      <div className="main-content">
        {cuacaSekarang && (
          <WeatherCard data={cuacaSekarang} satuan={satuan} />
        )}
